...
export const runtime = "nodejs"; // Force Node.js runtime for MongoDB and crypto operations
...
export async function POST(request: NextRequest) {
  try {
    ...
    const html = `
      ...
      <p><strong>Adresse:</strong> ${personal.address}, ${personal.city}, ${
      personal.province
    } ${personal.postalCode}</p>
      
      <h3>Détails du véhicule</h3>
      ${vehicle.vin ? `<p><strong>VIN:</strong> ${vehicle.vin}</p>` : ""}
      <p><strong>Véhicule:</strong> ${vehicle.year} ${vehicle.make} ${
      vehicle.model
    } ${vehicle.submodel}</p>
